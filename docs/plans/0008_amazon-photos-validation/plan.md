...
- [ ] Macを再起動し、段階1の永続化によってマウントとバックアップが自動で戻ることを確認する。マウント位置が変わらないことも確認する（A06）。（このセッション自体がMac上で動作しているため未実施。利用者による再起動後、新しいセッションで確認する）
- [x] スリープ復帰後の追従を確認する。
...
| A06 | 基本 | Mac再起動後にマウントとバックアップが自動で戻る | 未実施。作業セッション自体がMac上で動作するため、利用者による再起動後、新しいセッションで確認する |
...
| A09 | 再構築 | SMBマウントをスクリプトで再現・判定できる | 段階1で導入・確認済み。SMB切断からの自動復帰は段階6のA05で確認した。再起動での復帰確認はA06として保留中 |
