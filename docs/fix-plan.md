...
`docs/requirements.md` を仕様、`docs/implementation-plan.md` を実装計画とし、2026-08-22 のコードレビューで検出した差分を解消するための計画。既存の安全性（上書きしない・検証前に公開しない・失敗を成功と報告しない）を弱める変更は行わない。

## 0. 方針

1. 安全性の欠陥（F1）とデータの置き場所を誤る欠陥（F2）を最優先で直す
2. 性能の問題（F3）は「検証の回数」ではなく「同じ検証の重複」だけを削る。要件が求める検証はすべて残す
3. 復旧不能状態（F4）と事前チェック順序（F5）を直し、要件 §10「1つでも失敗したら何もせずに中止」を成立させる
...
- 中断後に `_photo-manager/tmp/` に `.part` が残らず、正式名の不完全ファイルが存在しないこと（実装計画 Phase 4 の未カバー項目）
- ロックが解放されていること

**受け入れ条件**: 実装計画 Phase 0「SIGINT / SIGTERM でロックが解放され、作成中の一時ファイルが残らない」および終了コード1で終了することが、実際の転送経路を通るテストで確認できる。
...
要件 §7 は「MP4 の `CreateDate` はUTC保存」と明記している。UTC の壁時計に `tzinfo` を貼り替えると、UTC 値をそのまま現地時刻として扱うことになる。正しくは UTC として解釈してからオフセットへ変換する必要がある。
...
**受け入れ条件**: 実装計画 §7.3「QuickTime `CreateDate` と `TimeZone` のexiftool出力形式ごとの解釈」がテストと ADR で固定される。
...
要件が求める検証回数を超えて、同じデータを繰り返しハッシュしている箇所がある。4TB のミラー再実行や128GB満杯のカード取り込みで、実用時間が数倍になる。ただし、ミラー宛先の台帳公開前後の2回の検証は要件 §13 手順7・8が別々に要求しているため、見かけ上同じでも削除しない。
...
これにより、通常時の元ファイル再ハッシュは除去しつつ、要件 §9・§10 の「全対象ファイルがコピーと検証に成功、または宛先の同一内容を確認済み」「必要な全ファイルについて有効なレコードがある」と、eject 直前の宛先確認を維持する。

#### F3-b. `mirror` の宛先二重検証（`mirror.py:233-235`）

`_verify_target_data()` を呼んだ直後に `_verify_target()` が同じ `_verify_target_data()` を再実行している。ただし、要件 §13 手順8 は台帳置換後に全管理対象を再検証することを明示しているため、現行の2回は仕様上それぞれ意味があり、削除対象にはしない。
...
- 将来この2回を統合する場合は、先に要件 §13 と脅威モデルを改訂し、ADRで根拠を残す。この修正計画では行わない

#### F3-c. `mirror` のコピー元二重検証（`mirror.py:165` と `:206`）

要件 §13 のミラー手順は「1. パス・UUID・書込可否・容量の確認 → 2. ロック取得 → 3. コピー元台帳と全転送対象の検証」であり、ロック前の全件ハッシュは要件が求めていない。
...
**受け入れ条件**: 要件 §9・§10・§13 が求める検証はすべて残ったまま、仕様上意味のないコピー元の重複ハッシュと、台帳追記ごとの全台帳再読込が消えている。ミラー手順7・8の2回の宛先全件検証は仕様上必要なため維持する。
...
hard link 非対応または容量不足の宛先に対して、台帳の末尾修復と古い `.part` の削除を実行してから終了コード2で中止する。要件 §10「1つでも失敗したら、何もせずに中止する」と実装計画 Phase 1 に反する。
...
- 実装計画 Phase 6 の処理順（3. 状態を変更しない事前チェック → 4. ロック → 5. 台帳修復 → 6. 一時ファイル掃除）に合わせる
...
| e | `photo-verify` が読み取り専用マウントでもロックを省略せず動くよう、既存の常設ロックファイルを `O_RDONLY` で開いて `LOCK_SH` を取得するフォールバックを追加する。ロックファイルが無い、または共有ロックを取得できない場合は要件 §10 どおり終了コード2で中止し、ロックなしでは決して検証を続行しない。挙動を README に書く | `locking.py:25-41`, `verify.py:200`, `README.md` |
...
| M6 | 実機受け入れ（実装計画 §5 Phase 9 の手順を再実行） | テスト用SD・テスト用ボリュームで一連の確認が完了 |
...
3.8 に無い API を使っていないことは、実装計画 §2 のとおりレビューで担保する（`zoneinfo` / `str.removeprefix` / `dict` の `|` / `Path.is_relative_to` を使わない）。
...
- eject の条件（要件 §10）は緩めない。F3-a はこの実行で得た検証値を台帳照合へ受け渡して元ファイルの不要な再ハッシュを省くが、eject 直前の宛先再ハッシュは維持する
