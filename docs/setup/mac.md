...
通常実行はコピー完了後に、同じ配置計画の全保存対象について配置先のサイズとSHA-256を自動検証する。既存一致分はコピー前の照合結果を再利用し、新規コピー分だけ配置後に読み取る。`--dry-run` は検証もマニフェスト生成も行わない。コマンド完了前にはSDカードへの撮影、PhotoWorkの編集、主HDD上の整理を行わない。

コマンドはコピー結果と検証結果を分けて要約し、完全一致時だけ取り込み時点の `manifest_sha256` を表示する。詳細は `--log-dir` のJSONファイルへ保存する。`--log-dir` を指定しない場合は、実行時のカレントディレクトリに `.photo-copy-logs/` を作成する。コピーの衝突・失敗・未処理、または検証の欠損・不一致・読取不能・未解決があれば終了コードは1である。未対応のSony管理情報（`not-targeted`）とOS雑多ファイル（`excluded`）は許容する。引数や転送種別が不正、あるいはSSH接続先・主HDDの検査に失敗して実行できない場合は2である。
...
実行後は表示された要約だけを見る。対象の写真・動画について、コピー結果に `衝突`、`失敗`、`保存対象の未処理` がなく、検証結果が全件 `一致` で、`マニフェストSHA-256` が表示されれば主HDDへの取り込みと内容検証が完了である。詳細ログは実行時のカレントディレクトリの `.photo-copy-logs/` に自動保存される。コピー元のSDカードはこの時点で消去しない。

SDカードの管理用XML・DBなどの未対応形式は `対象外`、macOS補助ファイルは `除外` として記録され、対象メディアの成功を妨げない。写真・動画が未処理、衝突、失敗、または検証の欠損・不一致・読取不能になった場合は、コピー元を削除・整理せず、対応する詳細ログを開いて同じコマンドを再実行する。既に内容一致を確認できたファイルはスキップされるため、再実行で既存ファイルを上書きしない。検証成功後に主HDD上の写真を意図的に整理してよいが、古いコピー元に同じコマンドを再実行すると削除済みのファイルを再取り込みしうるため、必要な場合だけ実行する。
