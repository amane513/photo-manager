...
- 全件の内容検証（0010）、Immich（0011・0012）、第2 HDDへのバックアップ（0013）。
...
- [x] 削除前に、主HDD `/mnt/camera_archive/` 配下の全ファイルの一覧（パス・サイズ）を取得し、`validation.md` へ残す。コピー元（SDカード、iPhone本体、`~/Pictures/backup/`）に原本があることを、削除するファイルごとに確認する。
- [x] 数件を先に主HDDから削除し、Amazon側に残ることを確認する（A08）。残ることは許容する前提であり、削除の伝播は求めない。この確認はAmazon側の削除より前に行う。
- [x] 残りも主HDDから削除し、`/mnt/camera_archive/` 直下を空にする（`lost+found` など、写真以外で残すものは記録して残す）。macOSが作った `._*` や `.DS_Store` も削除する。
- [x] Macの `~/Pictures/PhotoWork/` からも同じテストコピーを削除する。0009はどちらにもテストコピーが残っていない状態から始める。
- [x] Amazon側を全削除する（利用者判断）。削除は不可逆であり、旧Macフォルダや過去のiPhone Auto-Save由来のデータも消えるため、削除前にアプリ上の件数と、ローカルに原本がないと思われるものの有無を確認して `validation.md` へ記録する。ゴミ箱（削除済み）に残る期間と、そこからの復元可否も記録する。（利用者が先行して実施したため、削除前の件数は未確認のまま。`validation.md` に保留事項として記録した）
- [x] 削除後、Amazon側が空であることを確認する。以降は「Amazonに存在する＝新しいライブラリルートから上がった」と判定してよい。
...
- [x] SDカードとiPhoneから代表メディア（JPEG、HEIC、ARW、動画、Live Photoの組）を `photo-copy` で主HDDへ配置する。`--destination-root` を省略し、新しい既定がライブラリルート配下になることを確認する（A12）。配置先が `photo-library/2026/2026-MM/{camera,smartphone}/` になることを確認する。
- [x] 現像済みJPEGとXMPは、配置したARWをdarktableで1件現像して作る（0007と同じ手順）。
- [x] 各形式がAmazon側に現れることを確認する（A01）。年フォルダを対象へ追加する操作をしていないこと（`photo-library` の指定だけで済んでいること）を明記する。
- [x] 各形式1件をAmazonからダウンロードし、開けること、元ファイルとサイズ・SHA-256が一致することを確認する（A02）。一致しない形式がある場合は、その形式の扱いを保留として記録する。
- [x] HEICの表示・再取得がJPEGと同等に扱えるかを確認する。差異がある場合は記録し、運用上の影響を判断する。
- [x] 同じコマンドを再実行し、内容一致による同一スキップが従来どおり動くことを確認する（A12）。

- [x] 主HDDにあるMP4・MOVがAmazon側に上がっていないことを確認する（A03）。動画は年月・機器別フォルダの中に静止画と混在しており、対象フォルダの指定では分離できない。除外はアプリ側の設定に依存する。
- [x] 動画が上がっている場合は、アプリ側に除外設定があるかを確認する。無い場合は、動画がAmazonへ上がることを許容するか、動画を別のライブラリルートへ分ける構成を検討するかを判断し、[proposal.md](../../proposal.md) 6章・8.1との整合を確認する。判断は記録し、必要なら後続プランへ回す。（動画は上がらなかったため対象外。判断不要）
- [x] 翌年分の扱いを確認する。`photo-library/2027/2027-01/camera/` に相当するフォルダを作ってファイルを1件置き、対象の指定を変えずにAmazonへ上がることを確認する（A04）。確認後、そのファイルとフォルダは削除する。

- [x] SMBを切断し、Amazon Photos Desktopがどう振る舞うか（停止するか、エラーを出すか、黙って対象を失うか）を確認する。再接続後にバックアップが再開することを確認する（A05）。（「DISCONNECTED」を明示表示。ただしSMB復旧の自動検知はせず、アプリの再起動が必要と判明。週次確認手順へ反映した）
- [x] Macを再起動し、段階1の永続化によってマウントとバックアップが自動で戻ることを確認する。マウント位置が変わらないことも確認する（A06）。
- [x] スリープ復帰後の追従を確認する。
- [x] コピー途中のファイルをAmazonが取り込まないことを確認する（A07）。`photo-copy` の一時ファイルはローカル転送が `.<最終名>.photo-copy-<16進>.part`、rsync over SSHがrsyncの既定の一時名であり、いずれもドットで始まる隠しファイルである（[src/photo_copy/local.py:54](../../../src/photo_copy/local.py#L54)）。大きめのファイルを転送しながらAmazon側の状態を観察する。
- [x] 進捗と停滞を週次でまとめて確認する方法（アプリ内のどの画面で何を見るか）を決める。
...
- [x] 0009・0010・0013へ渡す前提（ライブラリルートの位置、Amazonの対象フォルダの指定、マウント方式、主HDDに残っているもの、残した未確認事項）を `validation.md` に明記する。
...
| ID | 区分 | 確認すること | 現況 |
|---|---|---|---|
| A01 | 基本 | JPEG・HEIC・ARW・現像済みJPEGがAmazonへ上がる | 確認済み。段階4で配置した代表メディア5件（JPEG・ARW・現像済みJPEG・HEIC単体・HEIC(Live Photo)）がすべてAmazon側に現れた |
| A02 | 基本 | 各形式をAmazonから再取得でき、元と一致する | 確認済み。5件ともダウンロードしてSHA-256とサイズが完全一致し、開けることも確認した |
| A03 | 基本 | 動画がAmazonの対象外である | 確認済み。段階4で配置したMP4・MOVがAmazon側に上がっていないことを確認した |
| A04 | 基本 | 対象フォルダの指定が年をまたいでも漏れない | 確認済み。`photo-library/2027/2027-01/camera/` にテストファイルを置き、対象指定を変えずにアップロードされることを確認した |
| A05 | 基本 | SMB切断・再接続後にバックアップが再開する | 確認済み（切断は明示表示。再接続はマウント自動復帰、Amazon Photos Desktop自体はアプリ再起動が必要と判明） |
| A06 | 基本 | Mac再起動後にマウントとバックアップが自動で戻る | 確認済み。再起動後、LaunchAgentが`/Volumes/CameraArchive`（マウント位置は不変）を自動マウントし、Amazon Photos Desktopのバックアップも自動で再開した |
| A07 | 基本 | コピー途中の一時ファイルをAmazonが取り込まない | 確認済み。転送中の一時ファイルの実在とドット始まりによる不可視性を直接観測し、Amazon側にも異常がないことを確認した |
| A08 | 基本 | HDDから削除した写真がAmazonに残る（許容の確認） | 確認済み。段階2でJPEG・HEIC各1件を先行削除し、Amazon側に残ることを確認した |
| A09 | 再構築 | SMBマウントをスクリプトで再現・判定できる | 確認済み。段階1で導入し、SMB切断（A05）・Mac再起動（A06）いずれからも自動復帰することを確認した |
| A10 | 再構築 | Amazon Photos Desktopの手動手順が `docs/setup/mac.md` にある | 未着手 |
| A11 | 基本 | iPhoneのAuto-Saveが無効である | 無効であると利用者が確認済み。記録のみ |
| A12 | 基本 | `photo-copy` が新しい既定の配置先へコピーでき、`check` と再実行時の同一スキップも従来どおり動く | 確認済み。`check`・実コピー・再実行時の同一スキップ（rsync-ssh、local、preserveレイアウト）すべて実機で確認した |
...
- 0009・0010・0013へ渡す前提と未解決事項が明確である。
